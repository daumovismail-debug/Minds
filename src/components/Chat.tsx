...
export function Chat() {
  ...
  useEffect(() => {
    if (hasResult && resultsScrollRef.current) {
  ...
  return (
    <div className="flex flex-col w-full mx-auto h-full" style={{ maxWidth: '42rem' }}>
      {/* TOP: results area (scrollable) or empty centered greeting */}
      <div
        ref={resultsScrollRef}
        className={`flex-1 min-h-0 px-4 ${hasResult ? 'overflow-y-auto' : 'overflow-hidden flex items-center justify-center'}`}
      ...
      >
        {!hasResult ? (
          <div className="text-center animate-fade-in">
            ...
            {saved && (
              <div className="mt-4 inline-flex items-center gap-1.5 text-xs text-emerald-300 animate-fade-in">
      ...
      {/* BOTTOM: input bar (fixed position by flex layout) */}
      <div
