...
const RU_LIST_VERBS =
  /\b(покажи?|пока\s+же|покаж[уы]?|выведи|выдай|вывод|открой|найди|посмотр[ею]|посмотри|вывод[иь]|показывай|открой-ка|покажи-ка)\b/;

const RU_LIST_OBJECTS =
  /\b(мысл|задач|запис|всё|все\b|срочн|выполнен|невыполнен|активн|сделанн)/;

const RU_LIST_QUERIES =
  /(что\s+у\s+меня|сколько\s+у\s+меня|какие\s+у\s+меня|какие\s+мои|где\s+мои|мои\s+(задач|мысл|запис|срочн))/;
...
const URGENT_PATTERNS =
  /(\bсрочн|\basap\b|немедленно|критичн|поскорее|\bгорит\b|\bгоряч|сегодня\s+обязательно|до\s+конца\s+дня|сейчас\s+же)/i;
